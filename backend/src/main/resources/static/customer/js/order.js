...
const OrderView = (function () {
    ...
    function render() {
        ...
        if (submitBtn) { submitBtn.disabled = false; submitBtn.onclick = submitOrder; }
    ...
    async function submitOrder() {
        ...
        try {
            ...
            const orderItems = items.map((item, idx) => {
                ...
                return {
                    menuId: item.menuId, quantity: item.quantity, unitPrice: item.unitPrice,
            ...
            Cart.clear();
            if (response.sessionId) { Auth.updateSessionId(response.sessionId); }
            App.showToast(`주문 완료! 주문번호: ${response.orderNumber}`, 'success');
            MenuView.refreshCartBadges();
            setTimeout(() => { window.location.hash = '#/menu'; }, 5000);
        } catch (e) {
            App.showToast(e.message || '주문에 실패했습니다', 'error');
        } finally {
            if (submitBtn) { submitBtn.disabled = false; submitBtn.innerHTML = '주문 확정'; }
        }
    }
